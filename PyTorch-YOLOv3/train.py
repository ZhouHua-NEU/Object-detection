...
from utils.utils import *       #自定义类，包含两个函数，用于输入txt文本信息作为变量
...
if __name__ == "__main__":
    ...
    # parse_data_config，load_classes是自定义的两个函数，来自于utils.utils,用于输入txt文本信息作为变量
    data_config = parse_data_config(opt.data_config)
    train_path = data_config["train"]
    valid_path = data_config["valid"]
    class_names = load_classes(data_config["names"])

    #Darknet()是定义在models中的模型，参数model_df定义了config/yolov3.cfg模型的存放的位置，Darknet会根据文件生成相应的网络模型
